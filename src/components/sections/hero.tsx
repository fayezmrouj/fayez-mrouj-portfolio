...
import { ArrowRight, Download, Mail, MapPin, ExternalLink, Phone, User } from "lucide-react";
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid items-center gap-10 lg:grid-cols-[1.15fr_0.85fr] lg:gap-16">
          ...
          >
            ...
            {/* Contact info card under the portrait */}
            <motion.div
              initial={{ opacity: 0, y: 16 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.55 }}
              className="mt-4 overflow-hidden rounded-2xl border border-border bg-card/95 backdrop-blur shadow-sm"
            >
              {/* Name strip with emerald accent */}
              <div className="flex items-center gap-3 border-b border-border bg-primary/5 px-4 py-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-primary/15 text-primary ring-1 ring-primary/20">
                  <User className="h-4 w-4" />
                </div>
                <div className="min-w-0">
                  <div className="font-display text-base font-bold leading-tight text-foreground truncate">
                    {profile.name}
                  </div>
                  <div className="text-[11px] text-primary font-medium leading-tight">
                    {profile.title}
                  </div>
                </div>
              </div>
              ...
              <div className="divide-y divide-border">
                <div className="flex items-center gap-2.5 px-4 py-2.5">
                  ...
                  <div className="flex flex-wrap items-baseline gap-x-3 gap-y-0.5 min-w-0">
                    <a
                      ...
                      className="text-[11.5px] text-foreground truncate transition-colors hover:text-primary hover:underline"
                    >
                      {profile.email}
                    </a>
                ...
                <div className="flex items-center gap-2.5 px-4 py-2.5">
                  ...
                  <div className="flex flex-wrap items-baseline gap-x-3 gap-y-0.5 min-w-0" dir="ltr">
                    <a
                    ...
                    <span className="text-muted-foreground/40 text-[11px]">·</span>
                    <a
                      href={`tel:${profile.phoneJOR2.replace(/\s/g, "")}`}
                      className="text-[11.5px] text-foreground truncate transition-colors hover:text-primary hover:underline"
                    >
                      {profile.phoneJOR2}
                    </a>
